...
**Docker Compose** は、`compose.yaml` に書かれた複数のコンテナを、**一括で**起動・停止するための仕組みです。このリポジトリでは次の 4 つがサービスとして定義されています。
...
| `nginx` | リバースプロキシ（画面と API を **1 つの URL** にまとめる） |
...
| **サービス** | Compose が管理する「ひとつのコンテナ役」。このリポジトリでは `postgres` / `api` / `web` / `nginx`。 |
...
次の URL を **順に** 試してみてください（**推奨は nginx 経由の 1 本の入口**です）。
...
慣れてきたら、次の URL でも個別にアクセスできます（CORS の設定が必要になることがあります）。

| 用途 | URL |
|------|-----|
| フロントのみ | [http://localhost:5173](http://localhost:5173) |
| API ドキュメントのみ | [http://localhost:8000/docs](http://localhost:8000/docs) |
| ヘルスチェックのみ | [http://localhost:8000/health](http://localhost:8000/health) |
...
スマホ・タブレット・別 PC から画面を確認したいときの方法です。**開発用**の手順なので、本番公開の設定ではありません。

### 前提

- `docker compose up` でアプリが動いていること。
- 確認するデバイスと、開発 PC が **同じ Wi-Fi（LAN）** にいること（社内ネットワークでも同様）。
- 入口は **nginx のポート 80**（`http://localhost` と同じ構成）を使うと、API も `/api` で同一オリジンになり、設定がいちばん簡単です。

### 方法 A: PC の LAN アドレスで開く（手軽）

1. 開発 PC の IP アドレスを調べる。
   - **Mac:** ターミナルで `ipconfig getifaddr en0`（Wi-Fi が `en0` でない場合は `en1` など）
   - **Windows:** `ipconfig` の「IPv4 アドレス」
2. タブレットのブラウザで `http://<IPアドレス>` を開く（ポート 80 のときは `:80` は省略可）。
   - 例: `http://192.168.1.23`
3. 画面に `/health` の結果が出れば成功です。

`VITE_API_URL=/api`（相対パス）のままなら、タブレット側でも API は **同じ IP の `/api`** に向くため、localhost 用の URL を書き換える必要はありません。

### 方法 B: VS Code / Cursor のポート転送を使う

エディタが Docker で公開したポートを検知し、**ポート** ビューに一覧表示します（Cursor でも同様です）。

1. サイドバーの **ポート**（Ports）を開く。表示されないときはコマンドパレットで「Ports: Focus on Ports View」を実行。
2. 一覧に **80**（nginx）があることを確認する。ラベルは `app (nginx)` と表示されます（[.vscode/settings.json](.vscode/settings.json) で付けています）。
3. ポート 80 の行を右クリックし、**ポートの可視性（Port Visibility）** を変更する。
   - **Public（公開）:** 同じ LAN 上の別デバイスから、表示されたアドレスでアクセスできる場合があります。
   - **Private（プライベート）:** この PC のブラウザ向け（通常の `localhost` 確認）。
4. **転送先アドレス（Forwarded Address）** に表示された URL を、タブレットのブラウザで開く。

リモート開発（SSH・Dev Containers など）のときは、ポート転送が特に便利です。ローカルで Docker Desktop を使っているだけの場合は、**方法 A** の方が単純なことが多いです。

### うまくいかないとき

| 症状 | 対処のヒント |
|------|----------------|
| タブレットでページが開かない | PC とタブレットが同じ Wi-Fi か確認。macOS のファイアウォールで Docker / 着信を許可。 |
| 画面は出るが API だけ失敗 | **nginx 経由**（`http://<IP>`）で開いているか確認。`:5173` だけ別ポートで開いていると CORS の追加設定が必要。 |
| `:5173` 直で LAN から開きたい | `.env` の `CORS_ORIGINS` に `http://<IPアドレス>:5173` を追加して Compose を再起動。 |
| Vite の「Blocked request」 | `apps/web/vite.config.ts` の `allowedHosts` が有効か確認（テンプレートでは有効）。 |

### 注意（セキュリティ）

- 同じ LAN にいる端末から、開発中のアプリにアクセスできる状態になります。
- カフェの共有 Wi-Fi などでは、必要なときだけ使い、終わったら `docker compose down` で止めてください。
...
| `NGINX_PORT` | nginx の入口ポート（デフォルト 80）。画面も API もここから使うのが基本です。 |
| `API_PORT` / `WEB_PORT` | デバッグ用に API / フロントへ **直接** アクセスするときのポート。 |
| `VITE_API_URL` | **ブラウザ**が API を呼ぶときのベース URL。普段は **`/api`**（相対パス）のままでよいです。nginx 経由で同一オリジンになります。 |
| `CORS_ORIGINS` | ブラウザからのリクエストを許可する **画面の URL の一覧**（カンマ区切り）。`:5173` へ直接開くときに必要です。 |
| `ROOT_PATH` | nginx 経由で `/api/docs` を開くときの OpenAPI 用ベースパス（通常 `/api`）。 |
| `VITE_USE_POLLING` | Mac などでファイル変更が反映されにくいときに `1` にする Vite 用の設定。 |

**補足（とても大切）:** 普段は **http://localhost**（ポート 80）だけ覚えれば十分です。画面は `/`、API は `/api/...` に nginx が振り分けます。コンテナの中ではサービス名 `api` で呼び分けますが、**ブラウザの JavaScript から `http://api:8000` のように書いても動きません**（名前解決できないため）。
...
3. ファイルを **保存**する。
4. ブラウザに戻り、数秒以内に表示が変わるか確認する（**http://localhost** で開いていること）。

変われば、Vite のホットリロードが効いています（nginx 経由でも WebSocket で反映されます）。
...
3. 保存する。
4. ターミナルのログに **再起動っぽい行** が出るか、`/api/docs` を再読み込みして反映を確認する。
...
1. **http://localhost**（nginx 経由）で開いているか確認する（推奨）。
2. `.env` の `VITE_API_URL` が **`/api`** になっているか確認する（`:8000` 直指定は nginx 未使用時向け）。
3. `docker compose ps` で `api` と `nginx` が **Up** か確認する。
4. `VITE_*` は **web コンテナ起動時**に読まれることがあるため、`.env` を変えたら **`docker compose down` してから再度 `up`** が確実です。

### ブラウザの開発者ツールに CORS エラーが出る

**http://localhost**（ポート 80）で画面と API の両方を開いていれば、同一オリジンのため通常 CORS は発生しません。

`:5173` など **フロントだけ別ポート**で開いているときは、API が `CORS_ORIGINS` に書かれた **画面の URL** からだけブラウザ通信を許可します。
...
- 迷ったら **http://localhost** だけ使うのがいちばん簡単です。
